...
class OrganizationRole(models.Model):
    ...
    logo = models.FileField(upload_to='organizations/organization_role/', blank=True)
...
class OrganizationCategory(models.Model):
    ...
    order = models.PositiveIntegerField(choices=PRIORITY.CHOICES, default=PRIORITY.MEDIUM)
    code = models.CharField(max_length=32, unique=True)
    role = models.ForeignKey(OrganizationRole, on_delete=models.PROTECT, related_name='organization_categories')
    description = models.TextField(blank=True)
    display_name = models.CharField(max_length=256, blank=True)
...
class Membership(models.Model):
    ...
    management_email = models.EmailField(
        blank=True, help_text='Management email of the organization used during the event'
    )
